...
def assess_bookings(
    ...
    for booking in bookings:
        ...
        can_reach = travel_hours <= hours_until_booking
...
def format_report(assessments: Sequence[BookingAssessment]) -> str:
    lines = [
        "VIN,Station,Distance_km,Travel_hours,Hours_until_booking,Can_reach"
    ]
    for item in assessments:
        lines.append(
            f"{item.vin},{item.station_name} ({item.station_id}),"
            f"{item.distance_km:.2f},{item.travel_hours:.2f},{item.hours_until_booking:.2f},{item.can_reach}"
        )
    return "\n".join(lines)
...
def run_report(
    ...
    assessments = assess_bookings(bookings, locations, stations, average_speed_kmh)
