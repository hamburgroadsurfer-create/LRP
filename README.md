...
2. Select the three **CSV** files:
   - `Bookings_Return_Today.csv` (VIN + return station)
   - `StationsNew.CSV` (station names with latitude/longitude)
   - GNSS export as CSV (VIN + GNSS lat/long). If Power BI gives you XLSX, please “Speichern unter” → CSV first so everything stays offline and Python-free. Column names with spaces or underscores are fine (e.g., `GNSS Latitude` or `gnss_latitude`).
